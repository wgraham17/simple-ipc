...
{
    /// <summary>
    /// 	An implementation used to send and recieve messages interprocess using Windows Messaging
    /// 	Applications may leverage this instance to register listeners on pseudo 'channels', and 
    /// 	receive messages broadcast using a <see cref="MessageBroadcaster"/>implementation on the same machine. Non-form based 
    /// 	application are not supported by this implementation.
    /// </summary>
    // ReSharper disable InconsistentNaming
    public sealed class MessageListener : NativeWindow
    // ReSharper restore InconsistentNaming
    {
        ...
        public MessageListener()
        {
            ...
            var p = new CreateParams
            {
                ...
                X = 0,
                Y = 0,
                Caption = string.Concat("SimpleIPC.", Guid.NewGuid().ToString()),
                Parent = IntPtr.Zero
            };
        ...
        {
            return string.Format("SimpleIPC.{0}", channelName);
        }
